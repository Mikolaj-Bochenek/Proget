...
{
    public static async Task<PagedResult<T>> PaginateAsync<T>(this IMongoQueryable<T> collection, PagedBase query)
        => await collection.PaginateAsync(query.OrderBy, query.SortOrder, query.CurrentPage, query.PageSize);
    ...
    public static IMongoQueryable<T> Limit<T>(this IMongoQueryable<T> collection, PagedBase query)
        => collection.Limit(query.CurrentPage, query.PageSize);
